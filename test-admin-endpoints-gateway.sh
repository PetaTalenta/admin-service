...
# Admin Service Endpoint Testing Script via API Gateway
# This script tests all admin-service endpoints through the API gateway
...
# Start testing
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Admin Service Testing via Gateway    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
...
response=$(api_call "POST" "/login" "{\"username\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\"}")
...
response=$(api_call "GET" "/profile" "" "$ADMIN_TOKEN")
if echo "$response" | grep -q '"success":true'; then
...
response=$(api_call "PUT" "/profile" "{\"full_name\":\"Super Admin Updated\"}" "$ADMIN_TOKEN")
if echo "$response" | grep -q '"success":true'; then
...
response=$(api_call "POST" "/logout" "" "$ADMIN_TOKEN")
if echo "$response" | grep -q '"success":true'; then
